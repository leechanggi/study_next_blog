...
const Card = React.forwardRef<HTMLAnchorElement, Type.CardProps>((props, forwardRef) => {
	const { href = '#none', title, content, createdAt, tags, imgSrc, ...rest } = props;
		...
		>
			{imgSrc && isValidURL(imgSrc) && (
				<AspectRatio.Root ratio={16 / 9} className='overflow-hidden'>
					<Image src={imgSrc} alt={title} width={360} height={202.5} className={cn('w-full')} priority />
				</AspectRatio.Root>
			)}
